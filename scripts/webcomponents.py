# NOTE: Execute this script from the root of the project directory

boilerplate = '''
export function BoilerPlate(...children: ForelChild[]): ForelElement {
    return new ForelElement("BoilerPlate", ...children)
}
'''
...
for tag in htmltags:
    finalData += boilerplate.replace("BoilerPlate", tag)

finalData.replace("var", "Var")

with open("./packages/web/index.ts", "a") as x:
    x.write(finalData)